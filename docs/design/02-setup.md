...
| **D2.3** | Readiness cards on `/` and `/m/{machine}` | **[Y-168](../../tracker.md#3-task-board)**, opened 2026-08-09 — every §3.1 check is visible without running a command, over a `yantrad` route [Y-163](../../tracker.md#3-task-board) deliberately did not ship. **The route landed 2026-08-09**: `GET /api/readiness` and `GET /api/machines/{name}/readiness`, off the daemon's own refresh sweep, answering `heartbeat` from the beats it holds — the cards are what is left | `crates/yantrad/src/api.rs`, `web/src/` (needs D1.2) |
