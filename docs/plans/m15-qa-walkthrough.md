...
| 2026-09-13 | **When the checklist appears, ruling (b).** The checklist showed only while no machine was online, so a real first install never saw it. Now **the setup checklist is the home page until the appliance has its ssh key and at least one machine is ready.** After that, the dashboard shows a small *Finish setup* card until the other steps are done or the person dismisses it. *Add a device* is always reachable from `/machines`. [Y-390](../../tracker.md) builds it. *Ready* is the seven checks a session needs, all present, as [D7](../design/07-m15-screens.md) §4.1 has it; GitHub and `yantra-agent` are optional. A fleet with a workspace has passed the checklist, so an asleep machine does not send it back. |
| 2026-09-13 | **Whose device it is ([Y-404](../../tracker.md)).** Beat 1 ticked for any new node, whoever owned it. Three rulings: **(1)** a node another Tailscale user owns shows as *shared from another account · not supported yet*. It is listed, not hidden, and never counts. **(2)** A tagged node belongs to the tailnet, not a person, so it is not yours either. It is listed with the reason and never counts, as ADR-0016 already refuses tagged callers. **(3)** The rule binds beat 1, the machines list and the setup gate. A node you do not own never counts toward *one machine ready* and is never offered to run a session. The appliance's own node is the reference, so it reads as yours unless it is tagged. |
