...
systemd), and two tests use it. `service_unit.rs` installs the appliance's units
under systemd as PID 1 and starts the real `yantrad` in a container that has no
`tailscale` — so it refuses exactly as it would while `tailscaled` is still
