...
| **Status** | ✅ **M0, M1, M2 and M3 all closed.** M3 closed 2026-07-31 on **real-agent evidence**: `up --agent claude`, `logs`, `status` and `down` all ran against a live Claude Code 2.1.220 on a real machine, ending in `stopped cleanly (exit 143)`. 128 tests, 4 deliberate `#[ignore]`s. Six invariants out of M3 (I-44…I-49) and Y-046 closed after two milestones open — the detail is in [docs/archive/m2-m3.md](docs/archive/m2-m3.md).<br>**Q1 and Q6 both answered by the owner on 2026-07-31**: Coder is not useful for this case (kill criterion cleared, Y-021 done), and Yantra is **personal-first** — open source for others later, which binds M4's scope.<br>**M4 — Web UI is broken down** into Y-069..Y-073 — [the plan](docs/plans/m4-web-ui.md) is a proposal, and **Q13** (does the CLI go over HTTP?) and **Q14** (what the UI is built with) are the owner's before code starts. Open elsewhere: **Y-066** (name the trust-prompt state, I-49), **Y-020** (the telemetry ADR, now unblocked and the last thing holding it), **Y-037** (release rehearsal), **Y-044** (session store, receding). |
...
| **Updated by** | Claude (session: Q1 and Q6 answered by the owner; tracker split per crate, Y-068; M4 planned) |
...
something else entirely — which is why they are worth more than the code they constrain. **46 live**,
numbered to I-49.
...
The three below bind no crate. They are about how this project *works* — what counts as evidence —
...
| --- | --- | --- | --- | --- | --- |
| Y-020 | Synthesise research → architecture decisions | 🔵 doing | claude | Y-010..Y-015 | ADR-0004 (runtime) ✅, ADR-0005 (library crate) ✅, ADR-0006 (transport) ✅, ADR-0007 (workspace schema) ✅. ADR-0009 (machine names) ✅, ADR-0010 (drop `branch`) ✅, ADR-0011 (Claude Code as a TUI) ✅. Remaining: the telemetry ADR, still unnumbered. **Unblocked since M2 closed** — it is the last thing holding this task at `doing`, and it decides what [`yantra-agent`](crates/yantra-agent/tracker.md) reports. |
...
| Y-069 | `yantrad serve` — bind to the tailnet or refuse | ⬜ todo | — | §7.1 of the plan | The daemon becomes an `axum` server, and the only interesting part is where it listens. With **Q6 answered — personal-first, no auth beyond Tailscale — the bind address *is* the security boundary** (R-22). It comes from `Self.TailscaleIPs` in the `tailscale status --json` document the inventory reader already parses, so Yantra grows no second copy of Tailscale's identity (§B2); verified 2026-07-31 that both a v4 and a v6 address are reported. **Fails closed**: no Tailscale, no address, no daemon — and deliberately **no `--bind` flag**, because a flag that can expose the API is a flag someone eventually passes. Plus `/healthz`, `tracing` (already a workspace dependency, still unused), and a clean `SIGTERM` shutdown for M7's supervisor. |
| Y-070 | The read model — refresh in the background, never on the request path | ⬜ todo | — | Y-069 | **The milestone's actual content.** `ssh.rs:175` sets `ConnectTimeout=10`, so an asleep or expired-key machine (I-39 — two of six here) costs ten seconds. That is fine for a CLI, where a human typed the command and is watching; a browser polls whether or not anyone is looking, so a handler that calls `sessions::list` per request turns one open tab into a permanent ssh storm. One background task per data class writes a snapshot; handlers read memory. **`ControlPersist=300` makes this a win rather than a tax** — any interval under five minutes keeps every master warm (M2 measured 20 ms warm against 150 ms cold), and since the `ControlPath` is per-user (`ssh.rs:71`) the **CLI reuses the daemon's masters**. Three honesty rules: a snapshot carries its own age, a machine that did not answer is reported as unreachable rather than dropped (Y-054's rule), and "not looked yet" is `Option` and not an empty `Vec` (I-47's lesson). |
...
| Decision | Blocked on |
| --- | --- |
| Machine telemetry: **push-heartbeat agent, 10s/30s** (R1+R5) | ~~M2~~ — **unblocked since 2026-07-31**, and now the only thing holding Y-020 open |
...
| Q13 | **Does the CLI start talking HTTP to the daemon, or stay in-process?** The M4 claim says "the same HTTP API **the CLI uses**", and the CLI uses no HTTP API — it calls `yantra-core` in-process. `crates/yantrad/CLAUDE.md` states the destination plainly (*every client talks to this and nothing else*), so the wording describes either M4 or the endpoint, and they differ. **(a)** two callers of one library, "same API" meaning the daemon exposes exactly what the CLI can express and no more. **(b)** the CLI becomes an HTTP client — one path to the fleet, and `yantra up` stops working without a running daemon. **Prior: (a), in writing.** (b) is the right long-term shape and the wrong move in the milestone that first proves the daemon can serve a page, since it would make the one working interface depend on the one that does not exist yet — and it is strictly harder to reverse. ADR-sized either way; if (a), the ADR should record what would later justify (b). | Y-069, and the shape of M4 | biswa |
...
| **R-22** | **The bind address is the entire security model.** Q6 removed auth, so a listener on the wrong interface is an unauthenticated read API over the fleet's shape — machine names, workspace names, repo paths. | Everything `yantrad` can read, readable by anything that can reach the port. | 🟡 **new with M4.** Mitigated by taking the address from Tailscale rather than from configuration, refusing to start without it, and shipping no flag that can override it. | A test that asserts the daemon **refuses**, not one that asserts it binds. |
