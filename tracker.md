...
| **Status** | ✅ **M0–M3 closed. M4 is underway.** The daemon exists: it binds only to this machine's Tailscale addresses and refuses to start otherwise (Y-069), and it refreshes a snapshot in the background so no handler ever awaits ssh (Y-070). Also closed today: **Y-020** at last, as [ADR-0013](docs/adr/0013-the-heartbeat-carries-only-what-placement-scores.md) (`proposed`); **Y-066**, naming the trust-prompt state without ever answering it; **Y-037**, the release matrix green on its first run, retiring R-19; and **Y-074/Y-076**, the README and [the architecture diagrams](docs/architecture.md). **151 tests**, 4 skipped. 47 invariants, numbered to I-50.<br>**Q1, Q6 and Q13 answered by the owner. Q14 — what the web UI is built with — is the only thing blocking Y-072/Y-073.**<br>**A scope audit against `brainstorm.md` found four things with no written reason (Y-077..Y-080)**, the largest being that **`yantra resume` does not exist** despite being one of the four verbs the founding document named, and the promise Q9 used to kill Aider. |
...
| **Updated by** | Claude (session: Q1/Q6/Q13 answered; tracker split per crate; M4 planned and its daemon half built; Y-020, Y-037, Y-066, Y-069, Y-070, Y-074, Y-076 closed; scope audit → Y-077..Y-080) |
...
| ID | Task | Status | Owner | Depends | Notes |
| --- | --- | --- | --- | --- | --- |
| Y-077 | `yantra resume <workspace>` | ⬜ todo | — | — | **Found by the scope audit, 2026-07-31, and it is drift rather than a deferral — no task, no ADR, no open question covers it.** [`docs/brainstorm.md`](docs/brainstorm.md) names four agent verbs: *Launch. Resume. Stop. View logs.* Yantra ships launch, stop, logs and status. The string `resume` appears in **zero** lines of `crates/*/src/*.rs`, confirmed. This is not a missing nicety: **Q9 killed Aider on exactly this ground** — *"Yantra's core promise is 'continue where you left off'; Aider has no session to continue"* — and Claude Code was chosen partly for having the strongest resume story. **The machinery already exists**: `agent.rs` presets the session UUID today, which is what makes resume possible at all. Only the verb is missing. The one recorded trace is [the M2 plan](docs/plans/m2-real-machines.md) §9, which notes the constraint that `--resume` from the wrong cwd fails outright — design notes for a feature nobody scheduled. |
| Y-078 | `yantra attach <workspace>` | ⬜ todo | — | — | **Found by the scope audit.** Attaching is a **printed hint you copy-paste** (`attach_hint` in `crates/yantra/src/main.rs`), not an action. That may well be right — the hint carries the resolved tmux path, the machine and the `TERM` that Y-052/Y-053/Y-058 fought for, and running `ssh -t` from inside the CLI means handing over the terminal — but **the reasoning is written down nowhere**, which makes it indistinguishable from an omission. Decide it, then record it either way. |
| Y-079 | A stale `ControlMaster` socket is never unlinked | ⬜ todo | — | — | **Found by the scope audit, and it is a hole in the transport everything runs on.** [ADR-0006](docs/adr/0006-ssh-exec-transport.md) defers R7's explicit-master recommendation (`-M -N -f` plus a per-host mutex, `ControlMaster=no` for clients) — **it records the deferral but not the cost.** R7 documents it: a stale control socket is never unlinked, so *"every subsequent exec repeats the message, forever."* Nobody has hit it because `ControlMaster=auto` self-heals, so this is latent rather than urgent. The fix is an unlink-on-stale check, not a redesign. At minimum, amend ADR-0006 so the cost is recorded beside the deferral. |
| Y-080 | The research index has drifted from the ADRs | ⬜ todo | — | — | **Found by the scope audit.** [`docs/research/README.md`](docs/research/README.md) omits R7 entirely and cross-references two ADR numbers since taken by different decisions (→ *ADR-0007 telemetry source*, → *ADR-0006 agent plugin interface*); [`04-workspace-prior-art.md`](docs/research/04-workspace-prior-art.md) points at ADR-0005/0006 for the workspace schema, which is **ADR-0007**. Exactly the defect §5 already caught and fixed once on 2026-07-30 — **the index drifts, the ADRs do not** — so the second occurrence argues for a link check in CI rather than a third manual sweep. |
