...
| Y-040b | Add podman to CI so the fixture cannot rot | ⬜ todo | claude | Y-031 | **Real gap:** the fixture self-skips when podman is absent, and CI has no podman — so today the integration test passes CI without ever running. Add a `podman --version` step to the `test` job and assert the output contains no `SKIPPED:`. |
