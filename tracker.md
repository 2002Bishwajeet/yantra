...
| **Status** | 🔵 **M1 in progress** — Y-045, Y-040, Y-040b done; Y-041 in review. Repo live at [2002Bishwajeet/yantra](https://github.com/2002Bishwajeet/yantra), CI green (and now genuinely runs the container fixture). |
...
| I-27 **[V]** | **Killing the local `ssh` does not kill the remote command** — multiplexed or not. It reparents to PID 1. Use a stdin-EOF watchdog in the payload, which means `ssh`'s stdin must be a **held-open pipe**: `Stdio::null()` or `-n` makes the watchdog fire instantly and every command dies at once. | Measured. `exec 9<&0` is load-bearing because the remote shell redirects a background job's stdin to `/dev/null`. Closing stdin then doubles as a clean cancellation primitive. | R7, Y-041 |
