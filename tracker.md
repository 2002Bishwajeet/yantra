...
| Y-033 | CI workflow (GitHub Actions) | 🟢 review | claude | Y-030, Y-032 | `.github/workflows/ci.yml` — five jobs (`fmt`, `clippy`, `test`, `deny`, `cross`), each one a `just` recipe so CI and the local gate cannot diverge. `just check` gained `deny`; new `just ci` = `check` + `appliance`. Cross job asserts the ELF really is aarch64 and uploads the appliance binaries. |
