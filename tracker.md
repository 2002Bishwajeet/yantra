...
| Y-038 | Consolidate the zig version pin | ⬜ todo | claude | Y-033, Y-034 | After both CI PRs merge, `0.16.0` is pinned in **two** workflows and declared by no repo file. `ci.yml` should also adopt the `.github/actions/setup-rust` composite that PR #3 introduced, so the toolchain lives in one place. |
| Y-039 | Add CI badge to README | ⬜ todo | claude | Y-033 | Skipped deliberately by the CI agent to avoid a concurrent-edit collision on README. |
