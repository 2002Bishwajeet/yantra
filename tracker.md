...
| Y-040b | Add podman to CI so the fixture cannot rot | ✅ done | claude | Y-031 | [PR #8](https://github.com/2002Bishwajeet/yantra/pull/8). **`ubuntu-24.04` runners ship podman preinstalled** (verified via `actions/runner-images`, then confirmed live: CI logged `podman version 4.9.3`) — no install step needed. `test` job gained a `podman --version` step (fails loudly if that ever stops being true) and now calls the new `just test-ci` recipe instead of `just test`; `test-ci` runs nextest with `--success-output=immediate` and fails the job if `SKIPPED:` appears in the output. Verified in the live run: `PASS [6.651s] (7/7) yantrad::ssh_fixture ssh_reaches_a_real_tmux_in_the_container` — a real container took 6.6s, versus ~0.005s for the self-skip path, and no `SKIPPED:` appeared anywhere in the log. `just test`, used by local `just check`, is untouched — still skip-friendly. |
