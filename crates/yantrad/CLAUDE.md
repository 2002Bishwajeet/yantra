...
`POST /api/workspaces/{name}/{up,down,resume,tokens}` and `POST /api/relay` — **the CLI's own verbs
and nothing more**, being `yantra new`, `edit`, `up`, `down`, `resume`, `tokens` and `relay`. The
daemon may do what `yantra` can already do, which is what stops it growing a richer API the CLI
cannot reach. A new verb here starts in the CLI, and `yantra relay` was written before this route
was.
