...
`yantra new`, `edit`, `up`, `down`, `resume`, `tokens`, `logs`, `repair`, `relay`, `github login` and
`github logout`. The daemon may do what `yantra` can already do, which is what stops it growing a
richer API the CLI cannot reach. A new verb here starts in the CLI, and `yantra relay` was written
before this route was.

**`github login` is the one write that answers before it is done** (Y-342, ADR-0023 §3). It
