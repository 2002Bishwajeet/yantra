...
| `tokens.rs` | I-46 through `logs.rs`, whose `locate` finds the same file — and the journal's own arithmetic trap: **one API response is written once per content block**, so a sum per record double-counts, and each record names the four counts twice (its totals, then `iterations`) |
