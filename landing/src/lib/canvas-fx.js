...
const CanvasFX = (function () {
  ...
  const CLOTH_DEFAULTS = {
    pin: 'top', wind: 3, speed: 0.5, amplitude: 30, drape: 40, brush: 2.05,
    brushSize: 150, damping: 1, light: 0.5, sheen: 0.1, shadow: 0.25,
    cornerRadius: 20, backing: [0.043, 0.031, 0.024], perspective: 1200,
  };
...
void main () {
  ...
  /** Hangs a texture on rippling fabric. `draw(ctx, w, h)` paints the cloth. */
  function createCloth(output, draw, options) {
    const SEG = 96, NODES = SEG + 1, DT = 1 / 120, WAVE_SPEED = 30;
    const STIFFNESS = 0.55, FORCE_GAIN = 5.0, BLEED = 48;
    ...
    const gl = output.getContext('webgl2', {
      alpha: true, depth: false, stencil: false, antialias: true, premultipliedAlpha: true,
    });
    ...
    function compile(type, text) {
      ...
      gl.compileShader(s);
      if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) console.error('Cloth shader:', gl.getShaderInfoLog(s));
      return s;
    }
    ...
    for (let y = 0; y < SEG; y++) {
      for (let x = 0; x < SEG; x++) {
        ...
        gridIndices[offset++] = a; gridIndices[offset++] = c; gridIndices[offset++] = b;
        gridIndices[offset++] = b; gridIndices[offset++] = c; gridIndices[offset++] = d;
      }
    }
    ...
    function syncSize() {
      ...
      const sw = Math.max(1, Math.round(wrapper.clientWidth * dpr));
      const sh = Math.max(1, Math.round(wrapper.clientHeight * dpr));
      if (source.width !== sw || source.height !== sh) { source.width = sw; source.height = sh; contentDirty = true; }
    }
    ...
    function uploadContent() {
      ...
      draw(source2d, source.width, source.height);
    ...
    for (let a = 0; a < NODES; a++) hangCurve[a] = Math.pow(a / SEG, 1.3);

    let simTime = Math.random() * 60, gust = 0.5, fieldEnergy = 1;
    ...
    function stepSim(dt) {
      ...
      const w1 = WAVE_SPEED * kb1, w2 = WAVE_SPEED * kb2;
      ...
      for (let y = 0; y < NODES; y++) {
        ...
        for (let x = 0; x < NODES; x++) {
          ...
          const ab = axisFor(x, y);
          const force = windAmp * rowForce[ab[1]] * colForce[ab[0]];
          const acc = c2 * lap - STIFFNESS * h + force;
          const next = 2 * h - hPrev[i] + dt2 * acc;
          let value = h + (next - h) * decay;
          if (value > 3.5) value = 3.5; else if (value < -3.5) value = -3.5;
          hNext[i] = value;
        }
      }
      for (let b = 0; b < NODES; b++) {
        ...
        else if (config.pin === 'right') { x = SEG; y = b; }
        hNext[y * NODES + x] = 0;
      }
    ...
    function touchImprint(delta, width, height) {
      ...
      const cellW = width / SEG, cellH = height / SEG;
      const radius = Math.max(config.brushSize, 12);
      const rx = radius / cellW, ry = radius / cellH;
      const gx = touch.x / cellW, gy = touch.y / cellH;
    ...
    function composeVertices(width, height) {
      ...
      for (let y = 0; y < NODES; y++) {
        ...
        for (let x = 0; x < NODES; x++) {
          ...
          zField[i] = amp * Math.tanh(h) + drape * hangCurve[axisFor(x, y)[0]];
        }
      }
      fieldEnergy = energy;
      for (let y = 0; y < NODES; y++) {
    ...
    let raf = 0, last = performance.now(), simDebt = 0;
    let dead = false, running = false, visible = true;
    ...
    function frame(now) {
      ...
      if (!motion.matches) {
        ...
        while (simDebt >= DT) { stepSim(DT); simDebt -= DT; }
      }
      composeVertices(width, height);
      render();
      if (motion.matches || (config.wind <= 0.001 && fieldEnergy < 0.004 && touch.s < 0.01)) {
        running = false;
        return;
      }
    ...
    const cache = rectCache(wrapper);
    function onMove(event) {
      const r = cache.current;
      const x = event.clientX - r.left;
      const y = event.clientY - r.top;
      if (touch.s < 0.01) { touch.x = x; touch.y = y; touch.vx = 0; touch.vy = 0; }
...
export const { createCloth } = CanvasFX;
