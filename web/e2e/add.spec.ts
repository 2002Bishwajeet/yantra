...
/** Y-390, walk-through §3: one guided flow per platform, each beat seen by the
 *  appliance and never declared. `adding` holds a device at every beat —
 *  on the tailnet only, joined, joined under another account, ready, and one
 *  whose install stops at sudo — and a node that arrives three seconds in. */

const COMMAND = 'curl -fsSL http://100.64.0.1:7717/join | sh'
const flow = (machine?: string) => `/add?platform=linux${machine ? `&machine=${machine}` : ''}`
...
test.describe('Add a device', () => {
  test('a device only on the tailnet: the join command, a link and a QR code for it', async ({ page, size }) => {
    ...
    await expect(beat(page, 'On the tailnet').getByText('done · new-box is on the tailnet as linux')).toBeVisible()
    const joined = beat(page, 'Joined')
    await expect(joined.getByText('waiting · run the join command once in a terminal on new-box')).toBeVisible()
    await expect(joined.getByText(COMMAND)).toBeVisible()
    await expect(joined.getByRole('img', { name: /QR code for .*\/add\?platform=linux&machine=new-box/ })).toBeVisible()
    await axe(page)
    await screenshot(page, 'add-device', 'adding', size)
  })

  test('a joined device goes ready when Check asks it', async ({ page }) => {
    await scenario(page, 'adding')
    await page.goto(flow('joined-box'))
    await expect(beat(page, 'Joined').getByText(/^done · joined-box joined/)).toBeVisible()
    await beat(page, 'Reachable over ssh').getByRole('button', { name: 'Check' }).click()
    await expect(beat(page, 'Reachable over ssh').getByText(/^done · Yantra reached joined-box over ssh/)).toBeVisible()
    await expect(beat(page, 'Ready').getByText('waiting · missing claude · Install adds what is missing')).toBeVisible()
  })

  test('a device joined under another account says so and stops', async ({ page }) => {
    await scenario(page, 'adding')
    await page.goto(flow('kept-box'))
    await expect(beat(page, 'Joined').getByText(/^stuck · kept-box joined as biswa, but the ssh config logs in there as yantra/)).toBeVisible()
    await expect(beat(page, 'Reachable over ssh').getByText('waiting · waits for the join')).toBeVisible()
    await axe(page)
  })
  ...
  test('a ready device ends at New session', async ({ page }) => {
    ...
    await expect(ready.getByText('done · ready-box is ready · tmux, git and claude are there')).toBeVisible()
  ...
  test('an install that stops at sudo gives each command with Copy', async ({ page, size }) => {
    ...
    const ready = beat(page, 'Ready')
    await expect(ready.getByText('waiting · missing tmux, claude · Install adds what is missing')).toBeVisible()
    await ready.getByRole('button', { name: 'Install' }).click()
    await expect(ready.getByText(/^waiting · installing on blocked-box/)).toBeVisible()
  ...
  test('ticks the first beat by itself when a new node joins the tailnet', async ({ page }) => {
    ...
    await expect(page.getByText(/waiting · waiting for a new Linux machine on the tailnet/)).toBeVisible()
    await expect(page.getByText('done · fresh-box is on the tailnet as linux')).toBeVisible({ timeout: 15_000 })
  ...
  test('ends a phone at the home screen, and says Windows is coming', async ({ page }) => {
    await scenario(page, 'adding')
    await page.goto('/add?platform=phone&machine=iphone')
    await expect(page.getByText('done · iphone is on the tailnet as iOS')).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Open the dashboard there' })).toBeVisible()
